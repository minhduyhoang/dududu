...
export class AppleAuthService {
  ...
  constructor(
    ...
    this.appleIdAuthKeyUrl =
      this.configService.get<string>("APPLE_AUTH_KEY_URL") ||
      "https://appleid.apple.com/auth/keys";
    this.appleIdUrl =
      this.configService.get<string>("APPLE_URL") ||
      "https://appleid.apple.com";
    this.appClientId = this.configService.get<string>("APPLE_CLIENT_ID");
  }

  private async validateTokenAndDecode(
    token: string,
  ): Promise<JwtTokenSchema | boolean> {
    ...
    const applePublicKey: { keys: Array<{ [key: string]: string }> } =
      resAxios?.data;
    ...
    const sharedKid: string = applePublicKey?.keys.filter(
      (x) => x["kid"] === kid,
    )[0]?.["kid"];
    ...
    if (res.iss !== this.appleIdUrl || res.aud !== this.appClientId)
      return false;
  ...
  public async authenticate(
    ...
    try {
      const jwt: JwtTokenSchema | boolean =
        await this.validateTokenAndDecode(idToken);
