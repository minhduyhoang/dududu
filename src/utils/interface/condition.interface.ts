...
class ConditionQuery {
  public create(data: any = {}, keySearch: string = 'name') {
    ...
    const dateFilterField = data.dateFilterField ? data.dateFilterField : 'createdAt';

    if (data.fromDate) {
      data.fromDate = moment(data.fromDate).format('YYYY-MM-DD');

      conditions[dateFilterField] = MoreThanOrEqual(new Date(data.fromDate + ' 00:00:00'));
    }
    ...
    if (data.toDate) {
      ...
      if (data.fromDate) {
        conditions[dateFilterField] = Between(new Date(data.fromDate + ' 00:00:00'), new Date(data.toDate + ' 23:59:59'));
      } else {
        conditions[dateFilterField] = LessThanOrEqual(new Date(data.toDate + ' 23:59:59'));
      }
    }
