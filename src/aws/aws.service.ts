...
export class AWSService {
  constructor(private configService: ConfigService, @InjectAwsService(S3) private readonly s3: S3) {
  ...
  async checkConnection() {
    this.s3.listBuckets((err, data) => {
      if (err) {
        ...
        const requiredBuckets = [this.configService.get<string>('AWS_BUCKET_NAME')];
        ...
        bucketList.map((bucketName) => {
          if (bucketChecks[bucketName] === 'required') bucketChecks[bucketName] = 'existed';
        });
  ...
  public async uploadFile(file: any, key: string, bucket: string = this.configService.get<string>('AWS_BUCKET_NAME')): Promise<any> {
    try {
  ...
  public async deleteFile(key: string, bucket: string = this.configService.get<string>('AWS_BUCKET_NAME')): Promise<any> {
    try {
      if (!key) return true;
